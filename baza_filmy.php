...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="style\stylebaza.css">
</head>
<body>
    <header>
	...
	$query = $db->query("SELECT id_filmu, tytul, dlugosc, link, rezyser.imie, rezyser.Nazwisko, kategoria.gatunek, premiera FROM filmyy,rezyser,kategoria WHERE rezyser.id_rezysera=filmyy.id_rezysera and kategoria.id_kategori= filmyy.id_kategori");
	$table = array();
	$i=0;
	...
	<script>
		...
		for (var i = 0; i < 5; i++) {
			...
			obj.id = "container";
			obj.innerHTML = "<div id='z1'><img src='"+tabFilmy[i].link+"'></div><div id='z2'>"+tabFilmy[i].tytul+"</div><div id='z6'>something</div><div id='z3'>"+tabFilmy[i].dlugosc+"</div><div id='z4'>"+tabFilmy[i].imie + " " + tabFilmy[i].nazwisko+"</div><div id='z5'>"+tabFilmy[i].gatunek+"</div><div style='clear:both;'></div>";
			document.querySelector("main").appendChild(obj);
		}
		var butt = document.createElement('div');
		butt.setAttribute('class', 'butt');
		document.querySelector("main").appendChild(butt);
		for (var i = 1; i < prz; i++) {
			
			var btn = document.createElement("BUTTON");   // Create a <button> element
			btn.innerHTML = i;
			btn.setAttribute("onclick", ("page(" + i + ");"));
		...
		function page(x){
			var start = (x-1)*5;
			var end;
			if ((dlg-start) >= 5) end = start+5;
			else {
				end = start+((dlg-start)%5);
			}
			...
			for (let i = start; i<start+5; i++) {
				if (i>=end) {
					...
					objects[j].innerHTML = "<div id='z1'><img src='"+tabFilmy[i].link+"'></div><div id='z2'>"+tabFilmy[i].tytul+"</div><div id='z6'>something</div><div id='z3'>"+tabFilmy[i].dlugosc+"</div><div id='z4'>"+tabFilmy[i].imie + " " + tabFilmy[i].nazwisko+"</div><div id='z5'>"+tabFilmy[i].gatunek+"<div style='clear:both;'></div></div>";
				}
				j++;
			}
