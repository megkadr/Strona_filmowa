...
session_start();
 ...
 if (isset($_POST['loguj'])) {
	...
	if (empty($username)) {
		$_SESSION['pusty_login'] = "login jest wymagany";
		$error = true;
	}
	if (empty($_POST['haslo'])) {
		$_SESSION['puste_haslo'] = "hasło jest wymagane";
		$error = true;
	}
	...
	//echo $password;
  	...
  	if (mysqli_num_rows($results) > 0) {
  	  ...
  	  header('location: felinime.php');
  		...
  		$_SESSION['zla_kombinacja'] = "zła kombinacja loginu i hasła";
  	}
  }
}
?>
    ...
    <main>
		<form method="post" action="login.php">
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" id="nick" placeholder="Wpisz Nick" name="nick">
				<?php
					if (isset($_SESSION['pusty_login']))
					{
						echo $_SESSION['pusty_login'];
						unset($_SESSION['pusty_login']);
					}
				?>
			</div>
			<div class="form-group">
				<label for="exampleInputPassword1">Hasło</label>
				<input type="password" class="form-control" id="exampleInputPassword1" name="haslo" placeholder="Hasło">
				<?php
					if (isset($_SESSION['puste_haslo']))
					{
						echo $_SESSION['puste_haslo'];
						unset($_SESSION['puste_haslo']);
					}
				?>
			</div>
			<button type="submit" class="btn btn-primary" name="loguj">Zaloguj</button>
			
			<?php
				if (isset($_SESSION['zla_kombinacja']))
				{
					echo $_SESSION['zla_kombinacja'];
					unset($_SESSION['zla_kombinacja']);
				}
			?>
			<p>
				Jeśli nie masz konta <a href="registration.php" style="color: grey">Zarejestruj się</a>
			</p>
		</form>
    </main>
